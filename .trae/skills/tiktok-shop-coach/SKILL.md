...
| 脚本编剧 | 自动触发（日志记录后）/ 手动触发（写脚本） | [workflows/script-writer.md](./workflows/script-writer.md) | [templates/douyin-script.md](./config/templates/douyin-script.md) |

**意图不明时**：询问用户选择上述角色之一。

> **日志记录员**为后台角色，每次对话结束时自动将对话内容按角色分析结果记录到 `TikTokShop/{年份}/{月份}/` 目录中。
> **脚本编剧**为后台角色，日志记录完成后自动将当日日志转化为抖音口播脚本，保存到 `TikTokShop/{年份}/{月份}/脚本/` 目录中。
